...
def plot_subplots(csv_file, csv_file2):
    ...
    box_plot = input("Do you want a box plot for delay? (1 for Yes, 0 for No): ").strip()
    if box_plot == '1':
        # Extract columns from the first CSV
        timestamp = data.iloc[:, 0]  # Timestamp
        delay = data.iloc[:, 1]  # Delay
        retransmissions = data.iloc[:, 2]  # Retransmissions
        buffer_length = data.iloc[:, 3]  # Buffer Length
        sig_no = data.iloc[:, 4]  # Signal Number
        plt.figure(figsize=(6, 5))
        plt.boxplot(delay, vert=True, patch_artist=True, boxprops=dict(facecolor='lightblue'))
